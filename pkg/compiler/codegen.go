...
func (c *codegen) Visit(node ast.Node) ast.Visitor {
		...
		for _, spec := range n.Specs {
			...
			case *ast.ValueSpec:
				...
				if len(t.Names) == 2 && len(t.Values) == 1 && n.Tok == token.VAR {
					...
					if isMapKeyCheck, mapType = c.checkGetMapValueWithOKFlag(t.Values[0]); isMapKeyCheck {
	...
	case *ast.AssignStmt:
		...
		if len(n.Lhs) == 2 && len(n.Rhs) == 1 && (n.Tok == token.DEFINE || n.Tok == token.ASSIGN) {
			...
			isMapKeyCheck, mapType = c.checkGetMapValueWithOKFlag(n.Rhs[0])
		}
...
func (c *codegen) checkGetMapValueWithOKFlag(expr ast.Expr) (bool, *types.Map) {
